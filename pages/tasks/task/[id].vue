...
const sendOnChain = async () => {
  ...
  if (!isErrors) {
    ...
    const getValuesOnChain = {
      ...
      [ON_CHAIN_CONDITION_PARTS.TOKEN_NAME]: onChainValues.value?.tokenName,
...
const validationCheck = (key, isTriggered) => {
  ...
  switch (key) {
    case 'contractAddress':
      if (fieldInTheFormScope(ON_CHAIN_CONDITION_PARTS.CONTRACT_ADDRESS, selectedConditionValue)) {
        if (selectedConditionValue.value === ON_CHAIN_CONDITIONS.USER_HOLDS_TOKEN && onChainValues.value?.isNativeToken) {
          field.isError = false;
          field.text = '';
          break;
        }

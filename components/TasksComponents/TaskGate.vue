        ...
        <CreateGateItem class="mb-3" v-for="condition in  conditions" :item="condition"
            ...
            <div v-for="part in GATE_CONDITION_PARTS_MAP[condition.type] ">
                ...
                <div v-if="part === GATE_CONDITION_PREVIEWS.NFT_PREVIEW">
                    ...
                    :tokenId="condition.values?.tokenId" @update:nfName="value => changeConditionValue(condition.id, 'nftName', value)"
                    @update:imgUrl="value => changeConditionValue(condition.id, 'imgUrl', value)" />
                </div>
            </div>
        </CreateGateItem>
